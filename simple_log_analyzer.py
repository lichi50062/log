...
"""
Log執行時間分析工具 (僅處理當前資料夾)

這個腳本使用簡單的前綴後綴搜尋方式從log檔案中提取執行時間，
並計算統計數據（平均值、中位數、分位數等），將結果匯出到Excel檔案。

使用方法:
    python simple_log_analyzer.py log資料夾路徑 前綴字串 後綴字串 [選項]
...
def parse_args():
    """解析命令行參數"""
    parser = argparse.ArgumentParser(description="簡化版Log執行時間分析工具")
...
def get_all_files(log_dir):
    ...
    # 排除 log.bat 文件並篩選非目錄的文件
    all_files.extend([f for f in files if os.path.isfile(f) and os.path.basename(f).lower() != 'log.bat'])
...
def extract_times_from_directory(log_dir, prefix, suffix):
    """從資料夾中的所有檔案提取執行時間 (不包含子資料夾)"""
    all_files = get_all_files(log_dir)

    if not all_files:
        logger.warning(f"在 {log_dir} 中未找到任何檔案")
        return [], {}
...
def export_to_excel(times, overall_stats, file_stats, prefix, suffix, output_file):
    ...
    with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
        ...
        sheet = writer.sheets["詳細結果"]

        # 設置欄寬
        sheet.set_column(0, 0, 30)  # 檔案名稱
        sheet.set_column(1, 1, 10)  # 行號
        sheet.set_column(2, 2, 15)  # 執行時間
        sheet.set_column(3, 3, 100)  # 日誌行
...
def main():
    ...
    try:
        ...
        if not times:
            ...
            # 顯示詳細的調試信息
            print("\n=== 詳細診斷信息 ===")
            for file_name, file_debug_info in debug_info.items():
                for line_info in file_debug_info:
                    if line_info['contains_prefix'] and line_info['contains_suffix']:
                        line_text = line_info['line']
                        debug_line = line_info.get('debug_line', repr(line_text))
                        # 顯示行內容（限制長度以避免過長）
                        if len(line_text) > 80:
                            line_text = line_text[:77] + "..."

                        print(f"\n檔案: {file_name}")
                        print(f"行 {line_info['line_num']}: '{line_text}'")
                        print(f"  原始行: {debug_line}")
                        print(f"  包含前綴 '{prefix}': 是")
                        print(f"  包含後綴 '{suffix}': 是")

                        if 'prefix_positions' in line_info:
                            prefix_positions = line_info['prefix_positions']
                            suffix_positions = line_info['suffix_positions']

                            print(f"  前綴位置: {prefix_positions}")
                            print(f"  後綴位置: {suffix_positions}")
                            ...
                            for prefix_pos in prefix_positions:
                                ...
                                for suffix_pos in suffix_positions:
                                    if prefix_end <= suffix_pos:
                                        between_text = line_text[prefix_end:suffix_pos].strip()
                                        has_number = re.search(r'(\d+)', between_text) is not None
                                        print(
                                            f"  前綴結束位置 {prefix_end} 到後綴開始位置 {suffix_pos} 之間的文本: '{between_text}'")
                                        print(f"  該文本包含數字: {has_number}")
                                        if 'match_result' in line_info:
                                            print(f"  匹配結果: {line_info['match_result']}")
                                        if has_number:
                                            possible_matches = True

                            if not possible_matches:
                                print("  未找到前綴和後綴之間包含數字的有效區間")

            # 顯示部分原始檔案內容
            for file_name, file_debug_info in debug_info.items():
                if file_debug_info:
                    print(f"\n檔案 {file_name} 的前幾行內容:")
                    max_lines = min(10, len(file_debug_info))
                    for i in range(max_lines):
                        print(f"  {i + 1}: {file_debug_info[i]['line']}")
                ...
                for line_info in file_debug_info:
                    if 'line' in line_info:
                        ...
                        for number in numbers:
                            ...
                            possible_prefix = line[max(0, number_pos - 10):number_pos].strip()
                            possible_suffix = line[number_pos + len(number):min(len(line),
                                                                                number_pos + len(number) + 10)].strip()
                            if possible_prefix and possible_suffix:
                                suggestions.append({
            ...
            for suggestion in suggestions[:3]:
                suggestion_key = f"{suggestion['prefix']}|{suggestion['suffix']}"
                if suggestion_key not in shown_suggestions:
                    print(f"前綴: '{suggestion['prefix']}'")
                    print(f"後綴: '{suggestion['suffix']}'")
                    print(f"範例行: ...{suggestion['prefix']} {suggestion['number']} {suggestion['suffix']}...")
                    print()
                    shown_suggestions.append(suggestion_key)
